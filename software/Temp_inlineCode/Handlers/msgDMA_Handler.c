...
alt_u32* 	ptr2CurrCTRLStreamingDebugData[4] = {(alt_u32*)CURRCTRL_SYS_CURRCTRL_DEBUG_RAM_0_BASE,
												...
												(alt_u32*)CURRCTRL_SYS_CURRCTRL_DEBUG_RAM_3_BASE};
	...
	if ((PageCount == 0) && (TRUE))
		{
			...
			if (*(PC_to_NIOS_Shared_RAM_base_address + 16) == Registers.Node_Address.DataField)
				UpdateRegisterData(PC_to_NIOS_Shared_RAM_base_address + 17);
		}
	else
		{
			DMA_Stack_Entry.BytesToTransfer = 0; //4096;
	...
	switch (DMA_Stack[DMA_Read_StackPointer & 0x0f].InterruptFlag)
		{
...
#ifdef DEBUG_FIFO //CURRCTRL_SYS_CURRCTRL_GPIO_BASE
			...
			case FIFO_STREAMING_DEBUG_READ_3:
				IOWR_ALTERA_AVALON_PIO_SET_BITS(CURRCTRL_SYS_CURRCTRL_GPIO_BASE, 0x01 << 3);
				Clear_TP3();
				Clear_TP3();
				...
				IOWR_ALTERA_AVALON_PIO_CLEAR_BITS(CURRCTRL_SYS_CURRCTRL_GPIO_BASE, 0x01  << 3);
				break;
#endif
			default:
				break;
