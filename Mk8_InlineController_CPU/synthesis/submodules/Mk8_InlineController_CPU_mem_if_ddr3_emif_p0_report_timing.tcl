...
source "$script_dir/Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_report_timing_core.tcl"
...
foreach inst $instances {
	...
	set interface_type [Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_get_io_interface_type $all_dq_pins]
	...
	set io_std [Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_get_io_standard [lindex $dqs_pins 0]]
...
set tDCD 0.02
	...
	if {[llength $instances] <= 1} {
		set fbasename "${::GLOBAL_Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_corename}"
	} else {
		set fbasename "${::GLOBAL_Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_corename}_${inst_id}"
	}
	...
	foreach opc $opcs {
		...
		set model_corner [Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_get_model_corner]
		...
		if {!$in_gui && [array exists assumptions_cache] &&  [info exists assumptions_cache(${::GLOBAL_Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_corename}-$inst)] } {
			set assumptions_valid $assumptions_cache(${::GLOBAL_Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_corename}-$inst)
			...
			set assumptions_valid [Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_verify_flexible_timing_assumptions $inst pins $mem_if_memtype]
			set assumptions_cache(${::GLOBAL_Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_corename}-$inst) $assumptions_valid
		}	

		#######################################
		# Determine parameters and values that are valid only for this operating condition

		[catch {get_float_table_node_delay -src {SCALE_FACTOR} -dst {TOTAL_SCALE_FACTOR} -parameters {IO}} total_max_scale_factor]
		[catch {get_float_table_node_delay -src {SCALE_FACTOR} -dst {TOTAL_SCALE_FACTOR} -parameters {IO MIN}} total_min_scale_factor]
	        if {  ($total_max_scale_factor != "") && ($total_min_scale_factor != "") } {
		        set scale_factors(total) [expr $total_max_scale_factor - $total_min_scale_factor]
		} else {
			set scale_factors(total) 0
		}

		[catch {get_float_table_node_delay -src {SCALE_FACTOR} -dst {ODV_SCALE_FACTOR} -parameters {IO}} odv_max_scale_factor]
		[catch {get_float_table_node_delay -src {SCALE_FACTOR} -dst {ODV_SCALE_FACTOR} -parameters {IO MIN}} odv_min_scale_factor]
		if {  ($odv_max_scale_factor != "") && ($odv_min_scale_factor != "") } {
		        set scale_factors(odv) [expr $odv_max_scale_factor - $odv_min_scale_factor]	
		} else {
			set scale_factors(odv) 0
		}

		[catch {get_float_table_node_delay -src {SCALE_FACTOR} -dst {EOL_SCALE_FACTOR} -parameters {IO}} eol_max_scale_factor]
		[catch {get_float_table_node_delay -src {SCALE_FACTOR} -dst {EOL_SCALE_FACTOR} -parameters {IO MIN}} eol_min_scale_factor]
		if {  ($eol_max_scale_factor != "") && ($eol_min_scale_factor != "") } {
		        set scale_factors(eol) [expr $eol_max_scale_factor - $eol_min_scale_factor]	
		} else {
			set scale_factors(eol) 0
		}

		[catch {get_float_table_node_delay -src {SCALE_FACTOR} -dst {MEM_INTERFACE_SCALE_FACTOR} -parameters {IO}} emif_max_scale_factor]
		[catch {get_float_table_node_delay -src {SCALE_FACTOR} -dst {MEM_INTERFACE_SCALE_FACTOR} -parameters {IO MIN}} emif_min_scale_factor]
		if {  ($emif_max_scale_factor != "") && ($emif_min_scale_factor != "") } {
		        set scale_factors(emif) [expr $emif_max_scale_factor - $emif_min_scale_factor]		
		} else {
			set scale_factors(emif) 0
		}

		#######################################
		# Write Analysis

		Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_perform_flexible_write_launch_timing_analysis $opcs $opcname $inst $family scale_factors $interface_type $max_package_skew $period pins t summary MP IP board

		#######################################
		# Read Analysis

		Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_perform_flexible_read_capture_non_dqs_timing_analysis $opcs $opcname $inst $family $period $all_dq_pins pins t summary MP IP board

		#######################################
		# PHY and Address/command Analyses

		Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_perform_ac_analyses  $opcs $opcname $inst scale_factors pins t summary IP		
		Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_perform_phy_analyses $opcs $opcname $inst $inst_controller pins t summary IP

		#######################################
		# Bus Turnaround Time Analysis
		Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_perform_flexible_bus_turnaround_time_analysis $opcs $opcname $inst $family $period $interface_type $tJITper $tDCD $pll_steps pins t summary MP IP SSN board ISI
	...
	set summary [lsort -command Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_sort_proc $summary]

	set f -1
	if { [Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_get_operating_conditions_number] == 0 } {
		set f [open $fname w]

		puts $f "Core: ${::GLOBAL_Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_corename} - Instance: $inst"
	...
	post_message -type info "Core: ${::GLOBAL_Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_corename} - Instance: $inst"
	...
	foreach summary_line $summary {
		...
		if {$su != "--"} {
			set su [ Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_round_3dp $su]
		}
		if {$hold != "--"} {
			set hold [ Mk8_InlineController_CPU_mem_if_ddr3_emif_p0_round_3dp $hold]
		}
